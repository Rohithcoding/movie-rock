 ...
 * @returns The title of the movie, or 'N/A' if the movie is not provided.
 */
export const mutateMovieTitle = (movie?: Movie): string => {
  if (!movie) return "N/A";
  return movie.title;
};

/**
 * Returns the title of a TV show.
 *
 * @param tv The TV show to get the title for. Optional.
 * @returns The name of the TV show, or 'N/A' if the TV show is not provided.
 */
export const mutateTvShowTitle = (tv?: TV): string => {
  if (!tv) return "N/A";
  return tv.name;
};
